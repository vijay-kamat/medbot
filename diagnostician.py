...
def generate_confidence_score(draft_diagnosis, context):
    client = Groq()
    scorer_prompt = "You are a strict Data Auditor. Assign a 'confidence_score' (0-100) based on strict adherence of the Draft to the Guidelines. Respond ONLY with JSON: {\"confidence_score\": 85}"
    payload = f"### GUIDELINES\n{context}\n\n### DRAFT DIAGNOSIS\n{draft_diagnosis}"
    response = client.chat.completions.create(
        model="llama3-8b-8192", messages=[{"role": "system", "content": scorer_prompt}, {"role": "user", "content": payload}], temperature=0.0, response_format={"type": "json_object"}
    )
    return response.choices[0].message.content

def generate_peer_review(draft_diagnosis, context):
    client = Groq()
    reviewer_prompt = "You are a Clinical Peer Reviewer. Write a concise, 3-4 sentence paragraph critiquing the draft against the guidelines. Note any hallucinations or unverified claims. Do not write a score."
    payload = f"### GUIDELINES\n{context}\n\n### DRAFT DIAGNOSIS\n{draft_diagnosis}"
    response = client.chat.completions.create(
        model="llama3-8b-8192", messages=[{"role": "system", "content": reviewer_prompt}, {"role": "user", "content": payload}], temperature=0.2
    )
    return response.choices[0].message.content
